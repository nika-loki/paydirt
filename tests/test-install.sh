...
OUT="$("$INSTALL" --tool codex,cursor 2>&1)"
expect "installed: ~/\.codex/skills/gtm-sandbox"
expect "installed: ~/\.cursor/skills/gtm-sandbox"

echo "T5: unknown tool is rejected"
if OUT="$("$INSTALL" --tool 'agents; rm' 2>&1)"; then fail "malicious tool name accepted"; else ok "malicious tool name rejected"; fi
if OUT="$("$INSTALL" --tool nosuchtool 2>&1)"; then fail "unknown tool accepted"; else ok "unknown tool rejected"; fi

echo "T6: --remove uninstalls"
OUT="$("$INSTALL" --remove --tool codex 2>&1)"
